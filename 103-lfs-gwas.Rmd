...
symlinking fastqs in alt-demulti to alt-longfin and aligning against pacbio

## Check metrics 
```{r}
df<-read_csv("meta/alt-longfin.txt", col_names = c("SampleID","Aligned","Dedup","Cov"))
ddf<-m2 %>% left_join(df) %>% mutate(Path=paste0("data/alt-longfin/",SampleID,".sort.flt.bam"))
```
...
m100 %>% select(Path) %>% write_tsv(col_names = FALSE, file="bamlists/ .bamlist")
```
...
-bam bamlists/ .bamlist -ref /home/maccamp/genomes/GCF021917145.1/GCF_021917145.1_fHypTra1_genomic.fna \
