...
ddf
write_csv(ddf,"meta/lfs-life-history-meta.csv")
```
...
m500<-gdf %>% filter(Dedup > 5e5)
m500 %>% select(Path) %>% write_tsv(col_names = FALSE, file="bamlists/33.bamlist")
...
#redoing with 33 bamlist (5e5 reads)

srun -t 12:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 30 -bam bamlists/33.bamlist -ref /home/maccamp/genomes/longfin-chrom/lfs_final_scaff_assembly.fasta \
-out outputs/200/snps-wgs-5  \
-minMaf 0.05 -minMapQ 20 -minQ 20 -GL 1 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/200/snps-5.out 2> outputs/200/snps-5.err &

#can't because ref is gone. Do I need this? whatever, redoing in 303
```
