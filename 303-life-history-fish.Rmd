...
#ddf2 <- ddf2 %>% filter(X2==20131490)
m<-ddf2 %>% select(-`FALSE`, -X2) %>% as.matrix()
```

```{r}
pheatmap(m, labels_row = ddf2$X2, labels_col = m27$clust_name_100, cluster_cols = TRUE, cluster_rows = FALSE)
```
20131490
```{r}
dlong<-ddf2 %>% filter(X2==20131490) %>% pivot_longer(cols = 3:29) %>% rename(Scaffold="FALSE")
dlong$name<-m27$SampleID
dlong$Pheno<-m27$clust_name_100
dlong %>% group_by(Pheno, value) %>% summarize(Count=n())
```
Frequency in HS fish, Type 1: 93% (n=6)
Frequency in MS fish, Type 1: 67 (n=6)
Frequency in LS fish, Type 1: 50% (n=10)
