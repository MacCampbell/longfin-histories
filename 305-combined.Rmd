...
bcftools reheader -s ../../bamlists/combined.names -o plink-renamed.vcf plink.vcf
...
## LD
```{sh, eval=FALSE}
cat outputs/305/1mbseqs.txt  | while read line; do bcftools view -Ov -r $line outputs/305/filtered.vcf.gz > outputs/305/vcfs/$line.vcf; done;
bcftools view  outputs/305/filtered-1m.vcf.gz  -r 'lcl|scaffold_26':8.25e6- > outputs/305/vcfs/26-region.vcf
bcftools view  outputs/305/filtered-1m.vcf.gz  -r 'lcl|scaffold_11':17e6- > outputs/305/vcfs/11-region.vcf
...
as.ggplot(pheatmap(m, cluster_rows = TRUE, annotation_col = annot,
                   labels_col = m1$ID, labels_row = ddf2$X2, fontsize_row = 14, fontsize_col = 9)) +
