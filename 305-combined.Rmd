...
-minInd 69 -bam bamlists/combined.bamlist -ref /home/maccamp/genomes/longfin-polished/lfs_run3_scaffolds_final.fa \
...
#70630 variants and 100 people pass filters and QC.
...
m22<-m2 %>% left_join(miss, by=c("SampleID"="SampleID"))
...
t<-tidy$gt %>% separate(gt_GT, into=c("a1","a2"), sep="/") %>% mutate(Value=ifelse(a1 %in% c(0,1),as.numeric(a1)+as.numeric(a2), NA)) %>% select(Indiv, POS, Value)
...
colnames(ddf)<-c("Chrom","Position",m2$SampleID)


ddf<-ddf %>% filter(Chrom=="lcl|scaffold_26")
ddf2<-ddf %>% filter(Position %in% tops$Position)
missers<-miss %>% filter(Missingness > 0.1)

ddf2<-select(ddf2, -missers$SampleID)


m<-ddf2 %>% select(-Chrom, -Position) %>% as.matrix()
...
m22 %>% arrange(Sex) %>% group_by(Sex) %>% summarize(Median=median(Dedup))
```
...
as.ggplot(pheatmap(m, cluster_rows = FALSE, annotation_col = annot, color = viridis(3,option="magma", alpha=0.9), annotation_colors =  ann_colors,
                   labels_col = m22$SampleID, labels_row = ddf2$Position, fontsize_row = 12, fontsize_col = 8)) +
  #ggtitle("Heatmap of Longfin Smelt Variants on scaffold_26\n") + 
  theme(plot.title = element_text(hjust=0.5))
ggsave("outputs/305/supplemental-figure-s2.pdf", width=20, height=8)
ggsave("outputs/305/supplemental-figure-s2.jpeg", width=20, height=8)
...
```{r}
ddf<-read_tsv("outputs/305/snps-wgs-3.geno.gz", col_names = FALSE) %>% select(-X83)
ddf[ddf==-1]<-NA
