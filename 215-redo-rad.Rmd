...
```{r}
lh<-read_csv("../delta-smelt/metadata/2012-all-samples.csv") %>% mutate(Pheno=ifelse(aggregated_new_classes=="MIG",0,1)) %>% mutate(SexPheno=sex-1)
...
bcftools reheader --samples bamlists/2012.names -o outputs/215/renamed-05.vcf.gz outputs/212/plink-05.vcf.gz
...
bcftools +fill-tags outputs/212/renamed-05.vcf.gz  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.10' | bcftools view -i 'INFO/MAF > 0.05' > outputs/215/plink-05-filtered.vcf
...
```{r}
df<-read_tsv(file="outputs/215/assoc05-75-2.lrt0.gz") %>% mutate(log10p = -log10(dchisq(LRT, df = 1))) %>%
