...
```{r}
lc<-read.delim("outputs/300/vcfs/lcl|scaffold_26.ldf.ld",sep="",stringsAsFactors=FALSE) %>% as_tibble() %>% arrange(R2) %>%  filter(R2 >0.5)
...
```{r}
lc<-read.delim("outputs/300/vcfs/lcl|scaffold_11.ldf.ld",sep="",stringsAsFactors=FALSE) %>% as_tibble() %>% arrange(R2) %>%  filter(R2 >0.3)
...
```{r}
as.ggplot(pheatmap(m, labels_row = ddf2$X2, labels_col = s72$ID,fontsize_row = 14, fontsize_col = 9)) +
  ggtitle("Heatmap of Longfin Smelt Variants on scaffold_11\n") + 
  theme(plot.title = element_text(hjust=0.5))
ggsave("outputs/300/lfs-11-heatmap-1m.pdf", width=20, height=12)
```
